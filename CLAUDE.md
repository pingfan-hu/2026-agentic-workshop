...
- `slides/<deck>/_quarto.yml` — `project:` + `resources: figs/` + `metadata-files: [../_shared.yml]` + `format: touhou-revealjs`. ~10 lines (basics adds `include-in-header: resources/_header.html` for its deck-specific embedded-site JS; basics and safety each add a `theme:` list, see below).
- `styles/slides.scss` — **content-specific** styles for **all** decks, one file, sectioned per deck. Safety: the `.map-hot` hand cursor over clickable map shapes, the arrow forced back onto plotly's `.nsewdrag` layer so the bar panel doesn't read as clickable, `.dl-pill` (the source-bundle download button under the USA map), and `.brand-pill--charlatan` / `--faker` (the two named fake-data packages on the fake-vs-real slide). Basics: `.link-pill` (the Waza statusline link under the context meter, same chassis as `.dl-pill` in purple, with a trailing `arrow-up-right` because it opens a page rather than handing you a file) and `.brand-pill--caveman` (the 🪨 title pill linking the caveman plugin repo on the token-saving slide, an emoji-glyph variant like `--faker`). Every deck that needs any of it lists this one file; unused rules are inert, which is why one shared **deck** stylesheet is safe. **It stays separate from `styles/styles.scss` (the parent *website* theme), and merging them is not a class-naming problem, so renaming classes does not fix it.** A deck is a separate document in a separate Quarto project, so one shared file would be compiled into both sides. Measured on the basics deck: deck CSS 70,696 → 115,794 bytes (+64%) and 135 dead website-component rules, plus three breakages no rename can address, because they are not classes: `body { font-family: … serif }` and `a:hover { text-decoration: underline }` are bare element selectors, and `$code-color` is an `scss:defaults` variable that overrode the deck theme's own. **Content-specific styling goes in `styles/`, never in the touhou extension**, which stays a reusable theme: the extension is shared with the canonical copy at `~/Documents/GitHub/personal/quarto-ext/touhou/` and the nine decks under `~/Documents/GitHub/slides/`, and all eleven copies are kept byte-identical (verify with `md5 -q` across them). The dividing line is whether a rule names a subject: the `.brand-pill` chassis, `.practice-box`, the `.divider-*` slides and `.thanks-card` are reusable and belong in the extension; a specific package's logo or a specific bundle's download button does not. When adding to the extension, propagate to all eleven copies in the same pass, or the drift comes back. Two mechanics make this work across the nested-project boundary, and both are load-bearing:
  - It is pulled in as a **theme layer**, not via `css:` — each deck's `_quarto.yml` sets `theme: [_extensions/touhou/touhou.scss, ../../styles/slides.scss]`. Theme sass is *compiled into* the deck's stylesheet, so a file outside the deck project is fine. A `css:` entry is only linked and copied, and Quarto copies nothing across a project boundary: it emits the `../../` href and leaves a 404 (symlinking the file in fails differently, since Quarto copies the symlink itself into `_site`, where it dangles, then errors with `AlreadyExists` on the next render). Naming `theme` in the deck **replaces** the extension's value, hence listing `touhou.scss` first.
  - It needs a `/*-- scss:rules --*/` boundary marker, or Quarto rejects it as a theme file.
  - Also note `styles/styles.scss` cannot serve this purpose: it is the parent *website* stylesheet, and decks render as separate projects inside an iframe, so it never loads there.
...
3. **Two rules in `slides/_extensions/touhou/touhou.scss` finish the job.** `#title-slide.dark-centered { background-color: transparent !important; }` keeps the section transparent so the navy `data-background-color` layer shows behind the circular banner (without it, `.dark-centered`'s opaque navy fill would cover the area). Scoped to `#title-slide` so other `.dark-centered` slides keep their solid navy fill. `#title-slide { background-size: 28%; background-position: right 12% center; background-repeat: no-repeat; }` sets the banner's size and placement (percentages relative to the 1050x700 stage). The same block caps `> h1.title` and `> .quarto-title-authors` at `max-width: 58%`: the banner is a background image and takes no layout space, so without the cap a long title runs straight under the circle instead of wrapping. That 58% is derived from the size and position above, so re-derive it if either changes.
...
- **Icons for agent concepts use Lucide `bot`, never `brain`.** Anything standing for an agentic model (the agent thinking, `/model`, an LLM) gets the robot. A brain reads as human, and the glyph is unpleasant besides. Applies to every deck
