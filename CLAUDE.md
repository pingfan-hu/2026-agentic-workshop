...
- **Practice panels overflow easily and Reveal clips rather than scrolls.** The stage is 1050x700 and a practice panel with four cards plus two footer lines runs past it. Both practice partials wrap their content in `.practice-box` (defined in `touhou.scss`), whose `font-size` scales the entire panel at once, since every size inside those partials is em-based. If a panel gains a card, shorten the copy first and drop that one font-size second. Do not trim the inline sizes individually: that is how the two panels drift out of sync. Checkpoint and demo panels are short enough that they keep their own inline wrappers
