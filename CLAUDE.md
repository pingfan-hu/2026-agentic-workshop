...
- `slides/<deck>/_quarto.yml` — `project:` + `resources: figs/` + `metadata-files: [../_shared.yml]` + `format: touhou-revealjs`. ~10 lines (basics adds `include-in-header: resources/_header.html` for its deck-specific embedded-site JS; safety adds a `theme:` list, see below).
- `styles/safety-slides.scss` — **content-specific** styles for the safety deck, currently three sections: the `.map-hot` hand cursor over clickable map shapes, the arrow forced back onto plotly's `.nsewdrag` layer so the bar panel doesn't read as clickable, `.dl-pill` (the source-bundle download button under the USA map), and `.brand-pill--charlatan` / `--faker` (the two named fake-data packages on the fake-vs-real slide). **Content-specific styling goes in `styles/`, never in the touhou extension**, which stays a reusable theme: the extension is shared with the canonical copy at `~/Documents/GitHub/personal/quarto-ext/touhou/` and the nine decks under `~/Documents/GitHub/slides/`, and all eleven copies are kept byte-identical (verify with `md5 -q` across them). The dividing line is whether a rule names a subject: the `.brand-pill` chassis, `.practice-box`, the `.divider-*` slides and `.thanks-card` are reusable and belong in the extension; a specific package's logo or a specific bundle's download button does not. When adding to the extension, propagate to all eleven copies in the same pass, or the drift comes back. Two mechanics make this work across the nested-project boundary, and both are load-bearing:
  - It is pulled in as a **theme layer**, not via `css:` — `slides/safety/_quarto.yml` sets `theme: [_extensions/touhou/touhou.scss, ../../styles/safety-maps.scss]`. Theme sass is *compiled into* the deck's stylesheet, so a file outside the deck project is fine. A `css:` entry is only linked and copied, and Quarto copies nothing across a project boundary: it emits the `../../` href and leaves a 404 (symlinking the file in fails differently, since Quarto copies the symlink itself into `_site`, where it dangles, then errors with `AlreadyExists` on the next render). Naming `theme` in the deck **replaces** the extension's value, hence listing `touhou.scss` first.
