...
// ---- Navbar: Setup split dropdown ("Full List" flyout) ----
// Quarto's native `menu:` would turn the whole Setup item into a dropdown
// toggle, killing its link. Instead, inject a small caret button to the right
// of the pill; clicking it opens a floating pill linking to the optional
// software page. The Setup link itself keeps working.
(function () {
  function init() {
    var link = document.querySelector('nav.navbar .navbar-nav .nav-link[href$="setup.html"]');
    if (!link) return;
    ...
    btn.setAttribute('aria-label', 'More setup pages');
    ...
    // Derive from the Setup href so it resolves under any deploy subpath.
    item.href = link.getAttribute('href').replace('setup.html', 'optional-software.html');
    item.textContent = 'Optional Software';
...
// ---- Recording pill (collapsed-navbar top row) ----
// Below 1100px the real Recording nav item folds into the hamburger dropdown,
...
(function () {
  function injectRegisterPill() {
    ...
    link.href = 'https://www.youtube.com/watch?v=kX1bzUHZWqc';
    link.target = '_blank';
    link.rel = 'noopener';
    ...
    link.appendChild(document.createTextNode('Recording'));
    container.appendChild(link);
  }
  if (document.readyState === 'loading') {
